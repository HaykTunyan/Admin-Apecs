...
const TableRowProductItem = ({
    ...
    useEffect(() => {
        const isSelected = productsIdsToPublish.includes(id);
    ...
    const handlePreview = (e: any) => {
        ...
        router.push(`/product-management/list/product-item/preview/${id}`);
    };
    ...
    const foundAttributes = Object.entries(tagEnums).map(([key, displayName]) => {
        const matchingAttribute = attributeValues.find(
            (item: any) => item.attribute?.type === displayName
        );
    ...
    return (
        <Link
            href={enabledEditSubCategory ? `/product-management/list/product-item/${id}` : ``}
            className={styles.tableRowProductItems}
        ...
        >
            ...
            <div
                ...
                onClick={(e) => handleToggleSelect(e)}
            >
            ...
            <div className={styles.tableBlock}>
                <span className={"text-16_20 text-default-primary"}>{sku}</span>
            </div>
            <div className={styles.tableBlock}>
                <span className={"text-14_faux text-tertiary"}>{mainProduct}</span>
            </div>
            <div className={styles.tableBlock}>
                {
                    status
            ...
            {
                foundAttributes.map(({key, value}) => (
                    <div key={key} className={styles.tableBlock}>
                        <span className={"text-14_faux text-tertiary"}>{value}</span>
                    </div>
                ))
            }
...
export default TableRowProductItem;
