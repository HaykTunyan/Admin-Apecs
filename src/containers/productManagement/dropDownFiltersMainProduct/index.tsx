...
const DropDownFiltersMainProduct = ({values, resetFilters, onResetComplete}: IDropDownAttributes) => {
    ...
    const handleSelectValue = (value: ICategoryFilter) => {
        ...
        params.set("mainProduct", value.id);

        router.push(`?${params.toString()}`, {scroll: false});
    };

    useEffect(() => {
        const filterParam = searchParams.get("mainProduct");
        if (filterParam) {
        ...
        <div className="dropDownContainerFilters">
            ...
            >
                <span className="text-16_20 text-grey-light line-clamp-1">
                    {selectedValue ? selectedValue.name : "Filter by main product"}
                </span>
                <svg
...
export default DropDownFiltersMainProduct;
